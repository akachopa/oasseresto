...
<x-layouts.app :title="'Retur '.$return->number">
    <x-page-header :title="$return->number"
                   :subtitle="$return->supplier?->name.' · '.$return->reasonLabel()"
                   :back="route('purchase.returns.index')">
        <x-slot:actions>
            <x-status-badge :status="$return->status" />

            @if ($return->status->isEditable())
                <a href="{{ route('purchase.returns.edit', $return) }}" wire:navigate class="btn-secondary">
                    <x-icon name="pencil" class="h-4 w-4" /> Ubah
                </a>

                <form method="POST" action="{{ route('purchase.returns.post', $return) }}"
                      onsubmit="return confirm('Posting retur dan keluarkan barang dari stok?')">
                    @csrf
