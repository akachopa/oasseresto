...
<x-layouts.app title="Umur Piutang">
    <x-page-header title="Umur Piutang"
                   subtitle="Bucket umur mengikuti kebijakan kredit company, bukan angka tetap."
                   :back="route('finance.receivables.index')" />

    <form method="GET" class="card card-pad mb-4 flex flex-wrap items-end gap-3">
        <div>
            <label class="label" for="customer">Customer</label>
            <select id="customer" name="customer" class="input w-auto">
                <option value="">Semua customer</option>
                @foreach ($customers as $id => $name)
                    <option value="{{ $id }}" @selected($customerId === (int) $id)>{{ $name }}</option>
                @endforeach
            </select>
        </div>
    ...
    <div class="card card-pad mt-4 space-y-3">
        <div class="flex flex-wrap items-center justify-between gap-2">
            <p class="section-title">Rincian per Customer</p>
                ...
                <tbody>
                    @forelse ($groups as $group)
                        <tr>
                            ...
                            <td class="text-right">
                                {{ $group['oldest_days'] > 0 ? $group['oldest_days'].' hari' : '-' }}
                            </td>
                            @foreach ($buckets as $bucket)
                                <td class="text-right">
                                    {{ Money::rupiah($group['buckets'][$bucket['label']] ?? 0) }}
                                </td>
                            @endforeach
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ count($buckets) + 4 }}" class="text-muted py-6 text-center">
